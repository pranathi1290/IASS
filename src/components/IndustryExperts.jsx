...
function ExpertCard({ expert }) {
  return (
    <article className="group relative h-80 min-w-[250px] overflow-hidden rounded-3xl border border-zinc-200 bg-zinc-100 sm:min-w-[280px] md:min-w-0">
      {expert.image ? <img src={expert.image} alt={`${expert.name}, ${expert.role}`} className="absolute inset-0 size-full object-cover transition duration-700 group-hover:scale-105" /> : <div aria-hidden="true" className="absolute inset-0 bg-gradient-to-br from-zinc-200 to-zinc-100" />}
      ...
      <div className="absolute inset-x-0 bottom-0 p-5">
        ...
        <p className="mt-1 text-sm text-zinc-200">{expert.role}</p>
      </div>
    </article>
  )
}
...
function IndustryExperts() {
  ...
  return (
    <section id="experts" className="overflow-hidden bg-white px-6 py-20 text-zinc-950 sm:px-10 sm:py-28 lg:px-12">
      <div className="mx-auto max-w-7xl">
        <div className="mx-auto max-w-3xl text-center">
          <p className="mb-5 text-xs font-bold uppercase tracking-[0.22em] text-brand">The people in your corner</p>
          <h2 className="text-4xl font-extrabold leading-[1.02] tracking-[-0.05em] text-zinc-950 sm:text-6xl">Learn from <span className="text-brand">industry experts.</span></h2>
